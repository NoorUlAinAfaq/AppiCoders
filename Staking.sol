...
contract StakingWithNFT is Ownable {
    ...
    function withdraw(uint256 amount) external {
        ...
        require(
            amount > 0 && amount <= staker.stakedAmount,
        ...
        returns (uint256)
    {
        ...
        uint256 timeElapsed = block.timestamp - staker.lastUpdateTime;
        return
            (staker.stakedAmount * REWARD_RATE * timeElapsed) /
            (REWARD_RATE_DENOMINATOR * SECONDS_IN_YEAR);
    }
    ...
    function _generateLPTokenMetadata(address user)
        internal
        view
        returns (string memory)
    {
        ...
        string memory currentTierName = staker.currentTier > 0 &&
            staker.currentTier <= tierNames.length
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                staker.timeWeightedScore.toString(),
                '"},',
                '{"trait_type": "Rewards Earned", "value": "',
                staker.rewardsEarned.toString(),
                '"},',
    ...
    function viewTokenURI(uint256 tokenId)
        external
        view
        returns (string memory)
    {
...
contract AchievementBadge is ERC721URIStorage {
    ...
    constructor(address _stakingContract)
        ERC721("Staking Achievement Badge", "SAB")
    {
        stakingContract = _stakingContract;
    }
    ...
    function _generateBadgeMetadata(uint256 tokenId, uint256 tier)
        internal
        view
        returns (string memory)
    {
        string memory tierName = tier < tierNames.length
            ? tierNames[tier]
            : "Unknown";
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                tierName,
                ' Achievement Badge", ',
                '"description": "This badge certifies achievement of the ',
                tierName,
                ' tier.", ',
